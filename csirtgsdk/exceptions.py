class CsirtgException(Exception):
    ...
    def __init__(self, request_id, *args, **kwargs):
        super(CsirtgException, self).__init__(request_id, *args, **kwargs)
        self.request_id = request_id


class ApiError(CsirtgException):
    """Errors produced by the Csirtg API."""

    def __init__(self, request_id, error, user_message_text, user_message_locale):
        """
        :param (str) request_id: A request_id can be shared with Csirtg
...
class HttpError(CsirtgException):
...
class InternalServerError(HttpError):
    """Errors due to a problem on Csirtg."""
